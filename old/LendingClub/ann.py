...
class NeuralNetwork(object):
	...
	def costPrime(self, X, y):
		...
		deltak = np.multiply(-(y-self.yHat), self.dsigmoid(self.z4))
		dEdWj2k = np.dot(self.a3.T, deltak)

		deltai = np.dot(deltak, self.W3.T)*self.dsigmoid(self.z3)
		dEdWj1j2 = np.dot(self.a2.T, deltai)

		deltaj = np.dot(deltai, self.W2.T)*self.dsigmoid(self.z2)
		dEdWij1 = np.dot(X.T, deltaj)

		return dEdWj2k, dEdWj1j2, dEdWij
	...
	def computeGradients(self, X, y):
		dEdWjk, dEdWjj, dEdWij = self.costPrime(X, y)
