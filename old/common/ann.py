...
class NeuralNetwork(object):
	...
	def costPrime(self, X, y):
		...
		dEdWjk = np.dot(np.vstack(self.a2), np.vstack(deltak))

		deltaj = np.dot(deltak, self.W2.T)*self.dsigmoid(self.z2)
		dEdWij = np.dot(np.vstack(X), np.vstack(deltaj).T)

		print(np.shape(deltak))
		self.residuals.append(y-self.yHat)
